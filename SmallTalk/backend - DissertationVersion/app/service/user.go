...
//用户注册
func (s *UserService) UserRegister(mobile, plainPwd, nickname, avatar, sex, memo string) (user model.User, err error) {
	...
	model.DbEngine.ID(registerUser.Id).Cols("token").Update(&registerUser)
	return registerUser, err
}
...
//用户更新个人信息
func (s *UserService) UpdateInfo(mobile, nickname, memo, sex string) (user model.User, err error) {
	updateUser := model.User{}
	model.DbEngine.Where("mobile = ?", mobile).Get(&updateUser)
	updateUser.Nickname = nickname
	updateUser.Memo = memo
	updateUser.Sex = sex

	model.DbEngine.Id(updateUser.Id).Cols("nickname, memo, sex").Update(&updateUser)
